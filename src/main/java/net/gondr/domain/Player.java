...
public class Player {
	...
	private Block[][] viewNextBoard;

	private int queueCurrent = 0;
	private int queueNowColor = 0;
	private int blockCount = 0;

	public Player(Block[][] board, Block[][] sideBoard) {
		this.board = board;
		this.viewNextBoard = sideBoard;
...
//		nowColor = rnd.nextInt(colorSet.length); //랜덤한 색상을 뽑는다.
		nowColor = current; // 지정된 색상이 나온다.

		draw(false); // 이 줄을 추가하고
		
		getNextBlock();// 다음 블럭 가져오고

		System.out.println("nowColor: " + nowColor + ", " + "current: " + current);
		System.out.println("queueNowColor: " + queueNowColor + ", " + "queueCurrent: " + queueCurrent);
	}

	public void nextBoard(Block[][] sideBoard) {
		this.viewNextBoard = sideBoard;
	}
	...
	public boolean down() {
		...
		if (!checkPossible()) {
			...
			nowColor = queueNowColor;
			rotate = 0;

			getNextBlock();
			System.out.println(current + ", " + nowColor);
			draw(false);
			// 다음 블럭 가져와서 대기열에 넣기.
	...
	private void drawQueueBlock() {
		for( int i=0; i<6; i++ ) {
			for(int j=0; j<6; j++ ) {
				viewNextBoard[i][j].setData(false, Color.WHITE);	
			}
		}
		for (int i = 0; i < shape[queueCurrent][rotate].length; i++) {
			int bx = (int) shape[queueCurrent][rotate][i].getX() + 2;
			int by = (int) shape[queueCurrent][rotate][i].getY() + 2;
			viewNextBoard[bx][by].setData(true, colorSet[queueNowColor]); // 제거나 색칠이냐
		}
	}

	private boolean isViewEndAlert = false;
	...
	private void getNextBlock() {
		if (checkGameOver() == false) {
			...
			isViewEndAlert = false;
			queueCurrent = rnd.nextInt(shape.length);
			queueNowColor = rnd.nextInt(colorSet.length);
			x = 5;
			y = 2;
			rotate = 0;
			drawQueueBlock();
